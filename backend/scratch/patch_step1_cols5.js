...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  const newText = `{/* Step 1: ยืนยันวัตถุดิบ */}
          {currentStep === 1 && (
            <div className="space-y-6">
              ...
              {/* 5 columns grid wrapper: BOM Bulk spans 2 columns, others span 1 */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 bg-slate-50 p-6 border border-slate-100 rounded-2xl shadow-xs w-full text-left">
                
                {/* 1. สารเคมี (BOM Bulk) - Spans 2 columns for clear, spacious reading */}
                {(() => {
                  ...
                  return (
                    <div className="lg:col-span-2 flex flex-col justify-between gap-3 p-5 bg-white border rounded-xl shadow-2xs hover:bg-slate-50/50 transition-all w-full group border-slate-100">
                      ...
                      {/* Spacious list of substances with breakdown */}
  ...
  console.log("Successfully patched Step 1 Grid to 5-columns with col-span-2 BOM card!");
} else {
