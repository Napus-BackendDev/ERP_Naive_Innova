...
import {
  ArrowLeft, Check, ChevronRight, ClipboardList, FileImage, PackageCheck, X, Phone, Mail, MessageCircle, MapPin
} from "lucide-react";
...
export default function TrackingPage({ params }) {
  ...
  return <main className={styles.page}><header className={styles.header}><div className={styles.brand}><img src="/logo-wordmark.png" alt="Naive Innova" className={styles.logoWordmark} /></div><span>ผลการค้นหาข้อมูลแท็ก</span></header><section className={styles.shell}><Link href="/track" className={styles.backLink}><ArrowLeft size={16} /> กลับไปหน้าค้นหา</Link><div className={styles.orderTitle}><div><p className={styles.eyebrow}>TRACKING NO.</p><h1>{tag}</h1><p>สั่งซื้อ 10 ก.พ. 2568 <i /> 3 รายการสินค้า <strong>กำลังดำเนินการ 1</strong> จาก 3 รายการจัดส่งสำเร็จแล้ว</p></div><PackageCheck size={35} /></div><section className={styles.customer}><p className={styles.sectionLabel}>ข้อมูลผู้สั่งซื้อ</p><div><Info label="ชื่อ" value="หจก. รุ่งเรืองอุตสาหกรรม (สำนักงานใหญ่)" /><Info label="สินค้า" value="กล่องบรรจุภัณฑ์" /><Info label="จำนวน" value="1,200 ชิ้น" /><Info label="เลขที่งาน" value="JOB-2568-0042" /><Info label="วันที่ปรับปรุง" value="15 ก.พ. 2568" /><Info label="กำหนดเสร็จโดยประมาณ" value="20 ก.พ. 2568" /></div></section><h2><ClipboardList size={18} /> รายการสินค้าในคำสั่งซื้อนี้</h2><div className={styles.products}>{products.map((product, index) => <ProductCard product={product} defaultOpen={index === 0} key={product.id} />)}</div></section><footer className={styles.footer}><div className={styles.footerInner}><div className={styles.company}><img src="/logo-wordmark.png" alt="Naive Innova" /><strong>Naive Innova</strong><span>OEM Pet Care &amp; Animal Health</span><span>บนเทคโนโลยีนาโนและสารสกัดธรรมชาติ</span><span>Spin-off จากคณะสัตวแพทย์ จุฬาฯ</span></div><div className={styles.contact}><b>ติดต่อเรา</b><a href="tel:0877149262"><Phone /> 087-714-9262</a><a href="tel:0948881184"><Phone /> 094-888-1184</a><a href="mailto:info@naiveinnova.com"><Mail /> info@naiveinnova.com</a><span><MessageCircle /> LINE: @naivepetcare</span></div><div className={styles.address}><b>ที่อยู่</b><span><MapPin /> 144/1 ต.สายเหนือ อ.พาน<br />&nbsp;&nbsp;&nbsp;&nbsp;จ.เชียงราย 57120</span><b className={styles.serviceTitle}>บริการ</b><a href="/track">ติดตามออเดอร์</a><span>สถานะการผลิต</span><span>คลังสินค้า</span></div></div><div className={styles.footerBottom}><small>© 2568 Naive Innova Co., Ltd. สงวนสิทธิ์ทุกประการ</small><small><i /> ระบบทำงานปกติ</small></div></footer></main>;
}