...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    metafile: true,
    loader: {
      // FORK: this DS ships JSX inside plain `.js` files (Next.js app, no
      // build/dist). esbuild's default `.js` loader can't parse JSX, so every
      // component fails to bundle. Output contract is untouched — this only
      // widens the parser. See cfg.libOverrides.
      '.js': 'jsx',
      '.svg': 'dataurl',
      '.png': 'dataurl',
      '.woff': 'dataurl',
