import fs from 'fs';

const path = 'C:/Users/asus/Desktop/naive/MES/frontend/src/components/dashboard/ProductionDetailView.js';
let code = fs.readFileSync(path, 'utf8');

// 1. Add Sparkles to imports
const oldImports = `import { 
  CheckCircle2, Camera, Video, Trash2, Check, Loader2, AlertTriangle, Beaker, PackageOpen, ListTodo, Award, ClipboardList, ChevronRight, UploadCloud,
  Tag, Droplet, BadgeCheck, Barcode, Clock, Box
} from "lucide-react";`;

const newImports = `import { 
  CheckCircle2, Camera, Video, Trash2, Check, Loader2, AlertTriangle, Beaker, PackageOpen, ListTodo, Award, ClipboardList, ChevronRight, UploadCloud,
  Tag, Droplet, BadgeCheck, Barcode, Clock, Box, Sparkles
} from "lucide-react";`;

code = code.replace(oldImports, newImports);

// 2. Insert helper functions above "const stepsList = ["
const insertMarker = 'const stepsList = [';
const insertIdx = code.indexOf(insertMarker);

if (insertIdx !== -1) {
  const helpers = `  const getMatchedPackagingItem = () => {
    if (!packagingItems || packagingItems.length === 0) return null;
    const packagingTypes = ["บรรจุภัณฑ์", "ขวดสเปรย์", "ขวดโฟม", "หลอดบีบ", "ขวด HDPE", "ซองฟอยล์", "ขวดเซรั่ม", "ขวดปั๊ม", "ขวดแชมพู", "ขวดแก้ว", "หลอดหัวปั้ม", "ขวดดรอปเปอร์"];
    return packagingItems.find(item => 
      (packagingTypes.includes(item.type?.name) || ["ขวด", "หลอด", "ซอง"].some(kw => item.name?.includes(kw)))
    );
  };

  const getMatchedCapItem = () => {
    if (!packagingItems || packagingItems.length === 0) return null;
    const capTypes = ["ฝา", "หัวปั๊ม", "หัวปั้ม", "ฝาขวด"];
    return packagingItems.find(item => 
      (item.type?.name?.includes("ฝา") || item.type?.name?.includes("ปั๊ม") || capTypes.some(kw => item.name?.includes(kw)))
    );
  };

  const getMatchedLabelItem = () => {
    if (!packagingItems || packagingItems.length === 0) return null;
    const labelTypes = ["กล่อง&ซอง", "กล่องไปรษณีย์", "กล่องกระดาษ", "ฉลาก", "สติกเกอร์"];
    return packagingItems.find(item => 
      (labelTypes.includes(item.type?.name) || ["กล่อง", "ฉลาก", "สติกเกอร์"].some(kw => item.name?.includes(kw)))
    );
  };

  const checkCapReady = (customer, items) => {
    const matched = getMatchedCapItem();
    if (!matched) return true;
    let qtyNeeded = 0;
    if (customer.orderedProducts) {
      if (Array.isArray(customer.orderedProducts)) {
        qtyNeeded = customer.orderedProducts.reduce((sum, p) => sum + (parseInt(p.quantityPcs || p.quantity) || 0), 0);
      } else if (typeof customer.orderedProducts === "object") {
        qtyNeeded = parseInt(customer.orderedProducts.quantityPcs || customer.orderedProducts.quantity) || 0;
      }
    }
    if (qtyNeeded === 0) qtyNeeded = 30;
    return matched.currentQuantity >= qtyNeeded;
  };

  const getBulkChemicalsInfo = () => {
    if (!cust.orderedProducts) return { names: "-", totalLiters: 0 };
    let productsList = [];
    if (Array.isArray(cust.orderedProducts)) {
      productsList = cust.orderedProducts;
    } else if (typeof cust.orderedProducts === "object") {
      productsList = [cust.orderedProducts];
    }
    let totalLiters = 0;
    const names = productsList.map(p => {
      const name = p.formulaName || p.name || "สารผสม";
      const qty = parseInt(p.quantityPcs || p.quantity || 0);
      const liters = (qty * 100) / 1000;
      totalLiters += liters;
      return \`\${name} (\${liters.toFixed(1)} ลิตร)\`;
    }).join(", ");
    return { names, totalLiters };
  };

`;
  
  code = code.substring(0, insertIdx) + helpers + code.substring(insertIdx);
  console.log("Successfully inserted helper functions!");
} else {
  console.log("Could not find insertMarker!");
}

// 3. Replace Step 1 JSX block
const startComment = '{/* Step 1: ยืนยันวัตถุดิบ */}';
const endComment = '{/* Step 2: QA รอบที่ 1 */}';

const startIdx = code.indexOf(startComment);
const endIdx = code.indexOf(endComment);

if (startIdx !== -1 && endIdx !== -1) {
  const oldStep1Text = code.substring(startIdx, endIdx);
  const newStep1Text = `{/* Step 1: ยืนยันวัตถุดิบ */}
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="border-b border-slate-100 pb-3">
                <h4 className="font-extrabold text-slate-800 text-sm flex items-center gap-2">
                  <ClipboardList className="h-5 w-5 text-green-600" />
                  <span>1. ยืนยันความพร้อมของวัตถุดิบและบรรจุภัณฑ์</span>
                </h4>
                <p className="text-[10.5px] text-slate-400 mt-1 font-semibold">ตรวจสอบความพร้อมของสารเคมีผสม ขวด ฝาขวด และป้ายสติกเกอร์ฉลาก เพื่อเริ่มต้นขั้นตอนการบรรจุ</p>
              </div>

              {/* 4 columns grid wrapper */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-slate-50 p-6 border border-slate-100 rounded-2xl shadow-xs w-full text-left">
                
                {/* 1. สารเคมี (BOM Bulk) */}
                {(() => {
                  const bulkInfo = getBulkChemicalsInfo();
                  return (
                    <div className="flex flex-col justify-between gap-3 p-4 bg-white border rounded-xl shadow-2xs hover:bg-slate-50/50 transition-all aspect-square w-full group border-slate-100">
                      <div className="flex justify-between items-center w-full">
                        <span className="text-[9.5px] text-slate-400 font-extrabold font-mono">BOM BULK</span>
                        <span className="text-[9px] font-extrabold text-green-700 bg-green-50 px-2 py-0.5 border border-green-200 rounded-lg">✓ พร้อมผสม</span>
                      </div>
                      
                      <div className="my-auto py-1 flex flex-col items-center justify-center gap-1">
                        <Beaker className="h-10 w-10 text-slate-400/80 transition-transform group-hover:scale-110 duration-200" />
                        <span className="text-[10px] text-slate-400 font-bold">{bulkInfo.totalLiters.toFixed(1)} ลิตร</span>
                      </div>

                      <div className="text-left mt-auto">
                        <p className="text-[11.5px] font-extrabold text-slate-700">เนื้อสารเคมีผสม</p>
                        <p className="text-[9.5px] text-slate-450 font-semibold mt-1 leading-normal truncate-2-lines">{bulkInfo.names}</p>
                      </div>
                    </div>
                  );
                })()}

                {/* 2. ขวดบรรจุภัณฑ์ (Packaging) */}
                {(() => {
                  const hasPack = checkPackagingReady(cust, packagingItems);
                  const matchedPack = getMatchedPackagingItem();
                  return (
                    <div className="flex flex-col justify-between gap-3 p-4 bg-white border rounded-xl shadow-2xs hover:bg-slate-50/50 transition-all aspect-square w-full group border-slate-100">
                      <div className="flex justify-between items-center w-full">
                        <span className="text-[9.5px] text-slate-400 font-extrabold font-mono">BOTTLE</span>
                        <span className={\`text-[9px] font-extrabold px-2 py-0.5 border rounded-lg \${
                          hasPack 
                            ? "bg-green-50 text-green-700 border-green-200" 
                            : "bg-red-50 text-red-700 border-red-200 animate-pulse"
                        }\`}>
                          {hasPack ? "✓ มีสต็อก" : "❌ สต็อกไม่พอ"}
                        </span>
                      </div>
                      
                      <div className="my-auto py-1 flex items-center justify-center">
                        {matchedPack && (matchedPack.imageUrl || matchedPack.image) ? (
                          <img src={getFileUrl(matchedPack.imageUrl || matchedPack.image)} className="h-16 w-16 object-contain mx-auto transition-transform group-hover:scale-110 duration-200" alt={matchedPack.name} />
                        ) : (
                          <PackageOpen className="h-10 w-10 text-slate-400/80 transition-transform group-hover:scale-110 duration-200" />
                        )}
                      </div>

                      <div className="text-left mt-auto">
                        <p className="text-[11.5px] font-extrabold text-slate-700">ขวดบรรจุภัณฑ์</p>
                        <p className="text-[9.5px] text-slate-450 font-semibold mt-1 leading-normal truncate-2-lines">{matchedPack ? matchedPack.name : "ขวดเปล่าตาม SKU"}</p>
                      </div>
                    </div>
                  );
                })()}

                {/* 3. ฝาขวด / หัวปั๊ม (Cap/Pump) */}
                {(() => {
                  const hasCap = checkCapReady(cust, packagingItems);
                  const matchedCap = getMatchedCapItem();
                  return (
                    <div className="flex flex-col justify-between gap-3 p-4 bg-white border rounded-xl shadow-2xs hover:bg-slate-50/50 transition-all aspect-square w-full group border-slate-100">
                      <div className="flex justify-between items-center w-full">
                        <span className="text-[9.5px] text-slate-400 font-extrabold font-mono">CAP / PUMP</span>
                        <span className={\`text-[9px] font-extrabold px-2 py-0.5 border rounded-lg \${
                          hasCap 
                            ? "bg-green-50 text-green-700 border-green-200" 
                            : "bg-red-50 text-red-700 border-red-200 animate-pulse"
                        }\`}>
                          {hasCap ? "✓ มีสต็อก" : "❌ สต็อกไม่พอ"}
                        </span>
                      </div>
                      
                      <div className="my-auto py-1 flex items-center justify-center">
                        {matchedCap && (matchedCap.imageUrl || matchedCap.image) ? (
                          <img src={getFileUrl(matchedCap.imageUrl || matchedCap.image)} className="h-16 w-16 object-contain mx-auto transition-transform group-hover:scale-110 duration-200" alt={matchedCap.name} />
                        ) : (
                          <Sparkles className="h-10 w-10 text-slate-400/80 transition-transform group-hover:scale-110 duration-200" />
                        )}
                      </div>

                      <div className="text-left mt-auto">
                        <p className="text-[11.5px] font-extrabold text-slate-700">ฝาขวด / หัวปั๊ม</p>
                        <p className="text-[9.5px] text-slate-450 font-semibold mt-1 leading-normal truncate-2-lines">{matchedCap ? matchedCap.name : "ฝาหรือหัวกดปั๊ม"}</p>
                      </div>
                    </div>
                  );
                })()}

                {/* 4. ฉลากสินค้า (Sticker/Label) */}
                {(() => {
                  const hasLabel = checkLabelReady(cust, packagingItems);
                  const matchedLabel = getMatchedLabelItem();
                  return (
                    <div className="flex flex-col justify-between gap-3 p-4 bg-white border rounded-xl shadow-2xs hover:bg-slate-50/50 transition-all aspect-square w-full group border-slate-100">
                      <div className="flex justify-between items-center w-full">
                        <span className="text-[9.5px] text-slate-400 font-extrabold font-mono">LABEL</span>
                        <span className={\`text-[9px] font-extrabold px-2 py-0.5 border rounded-lg \${
                          hasLabel 
                            ? "bg-green-50 text-green-700 border-green-200" 
                            : "bg-red-50 text-red-700 border-red-200 animate-pulse"
                        }\`}>
                          {hasLabel ? "✓ มีสต็อก" : "❌ สต็อกไม่พอ"}
                        </span>
                      </div>
                      
                      <div className="my-auto py-1 flex items-center justify-center">
                        {matchedLabel && (matchedLabel.imageUrl || matchedLabel.image) ? (
                          <img src={getFileUrl(matchedLabel.imageUrl || matchedLabel.image)} className="h-16 w-16 object-contain mx-auto transition-transform group-hover:scale-110 duration-200" alt={matchedLabel.name} />
                        ) : (
                          <Tag className="h-10 w-10 text-slate-400/80 transition-transform group-hover:scale-110 duration-200" />
                        )}
                      </div>

                      <div className="text-left mt-auto">
                        <p className="text-[11.5px] font-extrabold text-slate-700">ฉลากสินค้า</p>
                        <p className="text-[9.5px] text-slate-450 font-semibold mt-1 leading-normal truncate-2-lines">{matchedLabel ? matchedLabel.name : "สติกเกอร์สำหรับข้างขวด"}</p>
                      </div>
                    </div>
                  );
                })()}

              </div>

              <div className="pt-6 border-t border-slate-100 flex justify-end">
                <Button
                  disabled={uploading || !checkPackagingReady(cust, packagingItems) || !checkLabelReady(cust, packagingItems)}
                  onClick={handleConfirmStep1}
                  className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-all cursor-pointer shadow-md shadow-blue-100 flex items-center gap-1.5 active:scale-98"
                >
                  {uploading ? <Loader2 className="h-4.5 w-4.5 animate-spin" /> : <Check className="h-4.5 w-4.5" />}
                  <span>ยืนยันวัตถุดิบครบถ้วนเพื่อเริ่มผลิต</span>
                </Button>
              </div>
            </div>
          )
          }`;

  code = code.replace(oldStep1Text, newStep1Text);
  console.log("Successfully replaced Step 1 block!");
} else {
  console.log("Could not find start/end JSX comments for Step 1!");
}

fs.writeFileSync(path, code, 'utf8');
