...
// forked from design-sync lib/source-kit.mjs — plain-.js components with default exports
...
// FORK: this DS is plain JS (.js) — upstream only scans .tsx/.jsx.
const SRC_IMPL_RX = /\.(tsx|jsx|js)$/;
...
export async function resolvePackage(ctx) {
  ...
  // FORK: include .js — this DS ships plain-JS components.
  const srcFiles = srcRoot ? walk(srcRoot, (n) => /\.(tsx|jsx|js|mdx?)$/.test(n)) : [];
  ...
  if (!entry) {
    ...
    // FORK: every component here is `export default function <Name>()`, and
    // `export * from` does NOT re-export a default — without this the bundle
    // exports nothing and window.<GLOBAL> comes up empty. Re-export each
    // default under its file's PascalCase basename (dedup: first file wins).
    const seenDefault = new Set();
    const lines = [];
    for (const p of comps) {
      lines.push(`export * from ${JSON.stringify(p)};`);
      const base = basename(p).replace(/\.(tsx|jsx|js)$/, '');
      if (!/^[A-Z][A-Za-z0-9]*$/.test(base) || seenDefault.has(base)) continue;
      if (!/(^|\n)\s*export\s+default\b/.test(readText(p))) continue;
      seenDefault.add(base);
      lines.push(`export { default as ${base} } from ${JSON.stringify(p)};`);
    }
